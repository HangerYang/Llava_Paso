...
def main() -> None:
    ...
    responses = [line['response_base'] for line in data]# responses = [line['response_base'].split("### Response:\n")[1].strip() for line in data]
    ...
    for line, pred in zip(data, predictions):
        line['flagged'] = pred['flagged']
        num_total += 1
        if pred['flagged']:
            num_flagged += 1
        
    print(f"Settings: beta: {args.beta_val}, beavertails_threshold = {b_thresh}")
    print(f"Percent flagged: {num_flagged/num_total * 100}")
    print(args.eval_dataset)
    name = os.path.basename(args.eval_dataset)
    name = os.path.splitext(name)[0]
    with open(f'{args.output_dir}/{name}_test.json', 'w', encoding='utf-8') as f:
