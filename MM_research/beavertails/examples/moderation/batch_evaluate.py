...
def main() -> None:
    ...
    with open(args.eval_dataset, encoding='utf-8') as f:
        data = json.load(f)
    prompts = [line_part['prompt'] for key in data for line_part in data[key]]
    responses = [line_part['output'] for key in data for line_part in data[key]]
    ...
    with open(f'{args.output_dir}/batch_evaluation.json', 'w', encoding='utf-8') as f:
